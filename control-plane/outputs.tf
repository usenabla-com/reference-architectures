...
output "argocd_ui_ip" {
  description = "External IP address for the ArgoCD UI (if enabled)"
  value       = var.enable_argocd ? data.kubernetes_service.argocd_server[0].status[0].load_balancer[0].ingress[0].ip : null
  sensitive   = false
}

output "teleport_proxy_ip" {
  description = "External IP address for the Teleport Proxy (if enabled)"
  value       = var.enable_teleport ? data.kubernetes_service.teleport_proxy[0].status[0].load_balancer[0].ingress[0].ip : null
  sensitive   = false
}
